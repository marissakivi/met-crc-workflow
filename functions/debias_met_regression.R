    ...
    if(save.diagnostics){
      ...
      print(
        ggplot2::ggplot(data=dat.pred[dat.pred$Year>=mean(dat.pred$Year)-1 & dat.pred$Year<=mean(dat.pred$Year)+1,]) +
          ggplot2::geom_ribbon(ggplot2::aes(x=Date$yday, ymin=lwr, ymax=upr), fill="red", alpha=0.5) +
          ggplot2::geom_line(ggplot2::aes(x=Date$yday, y=mean), color="red", size=0.5) +
          ggplot2::geom_line(ggplot2::aes(x=Date$yday, y=obs), color='black', size=0.5) +
